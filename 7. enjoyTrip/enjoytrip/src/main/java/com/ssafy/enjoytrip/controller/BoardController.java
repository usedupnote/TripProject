...
public class BoardController {
	...
	public ResponseEntity<?> selectBoardByCategory(@PathVariable int boardCategoryId) {
		List<Board> boards = svc.selectBoardByCategory(boardCategoryId);
		System.out.println("categoryid" + boardCategoryId);
		if (boards != null) {
			return new ResponseEntity<List<Board>>(boards, HttpStatus.OK);
		}
		return new ResponseEntity<Void>(HttpStatus.NO_CONTENT); 
	...
	public ResponseEntity<?> updateViewCount(@PathVariable String id) {
		int result = svc.updateViewCount(id);
		if (result == 1) return new ResponseEntity<Integer>(result, HttpStatus.OK);
		return new ResponseEntity<Void>(HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	@GetMapping("/category/star/{userEmail}")
	public ResponseEntity<?> getAllStaredBoardByUserEmail(@PathVariable String userEmail) {
		List<Board> boards = svc.getAllStaredBoardByUserEmail(userEmail);
